...
| 参数字段 | 类型 | 必填 | 说明 |
| :--- | :--- | :--- | :--- |
| `messages` | List | **是** | **仅包含最新一条用户消息**。后端自动从 PostgreSQL 加载历史。 |
| `user_info` | Dict | **是** | 用户身份信息。 |
| `user_info.id` | Int | **是** | 学生ID (User Model ID)。 |
| `user_info.name` | Str | 否 | 学生姓名。 |
| `config` | Dict | 否 | **配置参数**。推荐使用 HTTP Header `X-Thread-ID` 传递 thread_id。如果无法使用 Header，则在此处传递 `configurable: { thread_id: "..." }`。 |
| `user_profile` | Dict | 否 | **(已弃用)** 推荐上下文记忆。后端已完全接管持久化，前端**无需**回传此字段。 |

*   **关于 `thread_id` (会话隔离)**:
    *   **推荐方式**: 在 HTTP Header 中添加 `X-Thread-ID: user_{id}`。
    *   **备选方式**: 在 Request Body 的 `config.configurable.thread_id` 中传递。
    *   **作用**: 唯一标识一个用户的会话，后端据此从 PostgreSQL 恢复 `chat_history` 和 `user_profile`。

*   **Apifox 请求示例 (Body: JSON)**:

#### 场景: 发送新消息 (推荐)
**Headers**:
`X-Thread-ID: user_123`
...
### 4.4 历史记录管理 (History Management)

提供对用户会话历史的查询和删除功能。

#### 1. 获取会话历史 (GET)
*   **路由**: `/student/history/{thread_id}`
*   **Method**: `GET`
*   **功能**: 获取指定 `thread_id` 的所有历史消息。
*   **Path参数**:
    *   `thread_id`: 会话唯一标识 (例如 `user_123`)。
...
*   **路由**: `/student/history/{thread_id}`
*   **Method**: `DELETE`
*   **功能**: **彻底删除**指定会话的所有持久化数据（包括 Checkpoint 和写入记录）。
*   **Path参数**:
    *   `thread_id`: 会话唯一标识 (例如 `user_123`)。
...
{
    "status": "success",
    "message": "History for thread user_123 deleted."
}
```
